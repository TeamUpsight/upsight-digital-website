...
const LOGO_URL = 'https://upsight.digital/images/logo-email.png';
...
function brandedHeader() { return `<tr><td style="background:#101c2a;padding:26px 32px"><img src="${LOGO_URL}" width="172" height="36" alt="Upsight Digital" style="display:block;width:172px;height:auto;border:0;outline:none"></td></tr>`; }
...
function insightBlocks(items: Array<{ title: string; description: string }>, color: string) {
  return items.map((item) => `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 10px"><tr><td style="padding:15px 16px;border-left:4px solid ${color};background:#f7f9f9;border-radius:0 8px 8px 0"><strong style="color:#17212b;font-size:15px">${escapeHtml(item.title)}</strong><br><span style="color:#52616b;font-size:14px;line-height:20px">${escapeHtml(item.description)}</span></td></tr></table>`).join('');
}

export function healthTeamHtml(data: HealthReport) {
  const answers = Object.entries(data.answers || {}).map(([key, value]) => `<tr><td style="padding:6px;font-weight:bold">${escapeHtml(key)}</td><td style="padding:6px">${escapeHtml(Array.isArray(value) ? value.join(', ') : value)}</td></tr>`).join('');
  return `<div style="font-family:Arial,sans-serif;max-width:680px;margin:auto"><h2 style="color:#00AD84">New Analytics Health Check</h2><p><strong>Email:</strong> ${escapeHtml(data.email)}</p><p><strong>Score:</strong> ${data.score}/100 — ${escapeHtml(data.maturity)}</p><h3>Answers</h3><table style="width:100%;border-collapse:collapse">${answers}</table></div>`;
}
...
export function healthConfirmationHtml(data: HealthReport) {
  ...
  return emailShell(`<tr><td style="padding:32px"><h1 style="margin:0 0 8px;color:#17212b;font-size:27px;line-height:35px">Your Analytics Health Score</h1><p style="margin:0;color:#52616b;font-size:15px;line-height:24px">${domain}</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:24px"><tr><td align="center" style="padding:25px;background:#101c2a;border-radius:12px"><div style="color:${scoreColor(data.score)};font-size:48px;line-height:52px;font-weight:bold">${data.score}<span style="font-size:22px">/100</span></div><div style="margin-top:8px;color:#ffffff;font-size:17px">${escapeHtml(data.maturity)} Analytics Maturity</div></td></tr></table>${sectionTitle('Category breakdown')}${categoryBreakdown(data)}${sectionTitle('Priority Risks')}${insightBlocks(data.risks.slice(0, 4), '#DC2626')}${sectionTitle('Recommended Next Steps')}${insightBlocks(data.recommendations.slice(0, 4), '#00AD84')}${improvement ? `${sectionTitle('Improvement opportunities')}${improvement}` : ''}${ctaButton('Book a Free Consultation')}</td></tr>`);
}
